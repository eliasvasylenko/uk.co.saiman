...
public class TreeItemImpl<T> extends TreeItem<TreeItemData<?>> {
  ...
  protected boolean hasChildrenContributions() {
    return getData().contributions(new TypeToken<TreeChildContribution<? super T>>() {}).anyMatch(
        c -> c.hasChildren(getData()));
  }

  protected List<TypedReference<?>> getChildrenContributions() {
    return getData()
  ...
  protected void rebuildChildren() {
    ...
    if (hasChildren) {
      if (isExpanded()) {
        List<TypedReference<?>> childrenData = getChildrenContributions();
