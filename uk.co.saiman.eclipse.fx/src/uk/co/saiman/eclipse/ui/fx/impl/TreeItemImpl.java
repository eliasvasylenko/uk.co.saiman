...
public class TreeItemImpl<T> extends TreeItem<Object> implements IAdaptable {
  ...
  void refreshContributions() {
    createAndUseContext(context -> {
      ...
      contribute(node, getModel(), context);
      getModel()
          .getContributions()
          .stream()
          .forEach(contributor -> contribute(node, contributor, context));
    });
  }

  private void createAndUseContext(Consumer<IEclipseContext> action) {
    IEclipseContext context = this.context.createChild();
    try {
      context.set(ListItems.ITEM_DATA, getValue());
      context
          .set(
              Invalidator.class,
              item
                  .getUpdate()
                  .<Invalidator>map(update -> () -> update.accept(getData()))
                  .orElse(() -> requestRefresh()));
  ...
  public void contribute(HBox node, CellContribution model, IEclipseContext context) {
    node.pseudoClassStateChanged(getPseudoClass(model.getElementId().replace('.', '-')), true);

    if (model.getContributionURI() != null) {
      ContextInjectionFactory.invoke(model.getObject(), AboutToShow.class, context);
    }
  }

  public void contribute(HBox node, Cell model, IEclipseContext context) {
    node.pseudoClassStateChanged(getPseudoClass(model.getElementId().replace('.', '-')), true);

    ListItemsImpl children = new ListItemsImpl(model.getChildren());
    ...
    this.children.addAll(children.getItems().collect(toList()));
