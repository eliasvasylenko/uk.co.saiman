...
public class CopleyNodeImpl implements CopleyNode {
  ...
  protected void ping() {
    comms.useChannel(channel -> channel.read());
  ...
  private byte[] receiveCopleyCommand(ByteChannel channel) {
    ...
    try {
      int read = channel.read(message_buffer);
      if (read != message_buffer.limit()) {
        throw comms
            .setFault(
                new CommsException("Response too short " + read + " of " + message_buffer.limit()));
      }
      message_buffer.flip();
    } catch (IOException e) {
      throw comms.setFault(new CommsException("Problem receiving command response", e));
    }
