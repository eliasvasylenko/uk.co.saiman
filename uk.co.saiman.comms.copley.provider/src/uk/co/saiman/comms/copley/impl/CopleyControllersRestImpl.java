...
import uk.co.saiman.comms.rest.ConnectionDTO.Status;
...
public class CopleyControllersRestImpl implements CopleyControllersRest {
  ...
  private ConnectionDTO getConnectionInfo(CopleyController comms) {
    ConnectionDTO connectionDto = new ConnectionDTO();

    connectionDto.channel = comms.getPort().getName();
    try {
      comms.reset();
      connectionDto.status = Status.CONNECTED;
    } catch (Exception e) {
      connectionDto.status = Status.FAULT;
      connectionDto.fault = e.getMessage();
    }
  ...
  public ControllerDTO getControllerInfo(CopleyController controller) {
    ...
    controllerDto.connection = getConnectionInfo(controller);
