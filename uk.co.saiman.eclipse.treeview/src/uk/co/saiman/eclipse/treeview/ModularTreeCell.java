...
public class ModularTreeCell extends TreeCell<TreeEntry<?>> {
  ...
  private TreeDropPosition getDropPosition(DragEvent event) {
    Point2D sceneCoordinates = ModularTreeCell.this.localToScene(0d, 0d);
    double height = ModularTreeCell.this.getHeight();
  ...
  public void startEdit() {
    ...
    super.startEdit();
    if (this.isEditing()) {
      getModularTreeItem().refreshContributions(() -> commitEdit(getItem()));
    }
  }
  ...
  public void commitEdit(TreeEntry<?> newValue) {
    ...
    getModularTreeItem().getModularParent().ifPresent(p -> p.refreshContributions(null));
    getModularTreeItem().refreshContributions(null);
  }
