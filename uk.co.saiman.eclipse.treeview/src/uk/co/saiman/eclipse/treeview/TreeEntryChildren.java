...
public interface TreeEntryChildren {
  void add(int index, TreeEntryChild<?> child);

  default <T> void add(TreeEntryChild<?> child) {
    add((int) stream().count(), child);
  }

  Stream<TreeEntryChild<?>> stream();

  default boolean isEmpty() {
    return !stream().findAny().isPresent();
  }

  default TreeEntryChild<?> get(int index) {
    return stream().skip(index).findFirst().get();
  }

  void remove(int index);

  boolean remove(TreeEntryChild<?> child);
}
