...
import static uk.co.saiman.msapex.chemistry.impl.ChemicalElementTile.Size.NORMAL;
import static uk.co.saiman.msapex.chemistry.impl.ChemicalElementTile.Size.SMALL;
...
public class PeriodicTableController implements Observable<ChemicalElementTile> {
  private Observable<ChemicalElementTile> selectionObservable;
  ...
  public void setPeriodicTable(PeriodicTable table) {
    ...
    for (ChemicalElementTile tile : elementTiles) {
      ...
      if (table != null && table.getElements().size() >= index) {
        element = table.getElement(index++);
      } else {
        element = null;
      }
